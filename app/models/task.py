...
class TaskResponse(BaseDB, BaseTask):
    ...
    additional_technician_names: list[str] = Field(
        default_factory=list, description="Names of additional technicians"
    )
    num_attachments: int = Field(default=0, ge=0, description="")
    assigned_by_name: str = Field(
        default="", description="Full name of the NOC/Admin who assigned this task"
    )
