...
class BaseIncident(SQLModel, ABC):
    client_id: UUID | None = Field(
        default=None, foreign_key="clients.id"
    )  # Client (SEACOM, Vodacom, etc.)
...
class Incident(BaseDB, BaseIncident, table=True):
    ...
    arrived_on_site_at: datetime | None = Field(
        default=None, sa_type=DateTime(timezone=True)
    )  # type: ignore
    temporarily_restored_at: datetime | None = Field(
        default=None, sa_type=DateTime(timezone=True)
    )  # type: ignore
    permanently_restored_at: datetime | None = Field(
        default=None, sa_type=DateTime(timezone=True)
    )  # type: ignore
...
class IncidentResponse(BaseDB, SQLModel):
    ...
    assigned_by_name: str = Field(
        default="", description="Full name of the NOC/Admin who assigned this incident"
    )
